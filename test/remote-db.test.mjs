...
for (const [name, variable] of [['PostgreSQL', 'TEST_POSTGRES_URL'], ['MySQL', 'TEST_MYSQL_URL']]) {
  test(`${name}: schema upgrades, exact upserts, project query, pagination and timezone`, { skip: !process.env[variable] }, async () => {
    ...
    try {
      ...
      const bounded = await queryDaily(db, new URLSearchParams({ startDate: '2026-09-01', endDate: '2026-09-01' }), null);
      assert.deepEqual(bounded.daily.filter(row => row.device === device), day.daily.filter(row => row.device === device));
      const metadata = await queryUsageMetadata(db);
      assert.ok(metadata.dimensions.devices.includes(device));
      assert.ok(metadata.dateRange.start <= '2026-09-01' && metadata.dateRange.end >= '2026-09-01');
